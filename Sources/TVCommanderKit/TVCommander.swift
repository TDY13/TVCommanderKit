...
extension TVCommander {
    ...
    public func getInstalledApps() {
        ...
        sendCommandOverWebSocket(createGetInstalledAppsCommand())
    }
    ...
    public func openApp(appId: String) {
        ...
        sendCommandOverWebSocket(createOpenAppCommand(appId: appId))
    }
    
    private func createGetInstalledAppsCommand() -> TVRemoteCommand {
        let params = TVRemoteCommand.Params(cmd: .request, dataOfCmd: TVRemoteCommand.Params.RequestData.installedApps, option: false, typeOfRemote: .remoteKey)
        return TVRemoteCommand(method: .query, params: params)
    }
    
    private func createOpenAppCommand(appId: String) -> TVRemoteCommand {
        let params = TVRemoteCommand.Params(cmd: .open, dataOfCmd: appId, option: false, typeOfRemote: .remoteKey)
        return TVRemoteCommand(method: .query, params: params)
    }
}

extension TVWebSocketHandler {
    ...
    func handleIncomingMessage(_ message: String) {
        ...
        do {
            let remoteCommand = try JSONDecoder().decode(TVRemoteCommand.self, from: data)
            switch remoteCommand.method {
            case .query:
                handleQueryCommand(remoteCommand)
            default:
                break
            }
        } catch {
    ...
    private func handleQueryCommand(_ command: TVRemoteCommand) {
        switch command.params.cmd {
        case .response:
    ...
    private func handleQueryResponse(_ command: TVRemoteCommand) {
        switch command.params.dataOfCmd {
        case let data as [String]:
            delegate?.tvCommander(self, didReceiveInstalledApps: data)
        default:
            break
        }
    }
